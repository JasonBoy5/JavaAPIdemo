...
//import com.yunkouan.wincc.util.ConversionUtil;

/**
 * Created by xt on 2018/6/7.
 */
public class Modbus4jRTU {
...
//          ModbusMaster master = getTcpMaster();
        ...
        try {
            ...
            //读取外部设备输入开关量状态  02
//            readDiscreteInputTest(master, slaveId, 0, 5);
            //读取保持寄存器数据 03
            readHoldingRegistersTest(master, slaveId, 0, 1);
            //读取外围设备输入的数据 04
...
//            writeCoilsTest(master, slaveId, 0, new boolean[] {true, true, true, true, true});
            //写入单个模拟量数据 06
            //writeRegisterTest(master, slaveId, 1, 102);
            //写入多个模拟量数据 16
//            writeRegistersTest(master, slaveId, 3, new short[] {1, 10, 100, 1000, (short) 65534});
...
//          batch.addLocator(4.1, BaseLocator.inputRegisterBit(slaveId, 0, 0));
        ...
        //高低字节颠倒
//        batch.addLocator(1, BaseLocator.holdingRegister(slaveId, 0, DataType.FOUR_BYTE_INT_UNSIGNED_SWAPPED)); //FOUR_BYTE_INT_UNSIGNED_SWAPPED     3700ff
        //高低字节不颠倒
          batch.addLocator(1, BaseLocator.holdingRegister(slaveId, 0, DataType.FOUR_BYTE_INT_UNSIGNED));        //FOUR_BYTE_INT_UNSIGNED      ff0037
        //读取浮点数
...
//        System.out.println(Long.toHexString((long) results.getValue(2)));
//        System.out.println(Long.toHexString((long) results.getValue(3)));
...
//            byte[] b1 = new byte[]{(byte)01,(byte)03,(byte)00,(byte)02,(byte)00,(byte)05,(byte)24,(byte)19};
//            ByteQueue byteQueue = new ByteQueue(b1);
//            System.out.println("byteQueue : "+byteQueue.toString());
//            request.write(byteQueue);
    ...
    public static ModbusMaster getAsiiMaster(){
        ...
        // 创建ModbusMaster工厂实例
        return new ModbusFactory().createAsciiMaster((SerialPortWrapper) serialParameters);
    }
    ...
    public static ModbusSlaveSet getAsciiSalve(){
        ...
        serialParameters.setPortOwnerName("ASCII");
        // 创建ModbusMaster工厂实例
        return new ModbusFactory().createAsciiSlave((SerialPortWrapper) serialParameters);
    }
    /**
     * @return
...
//        serialParameters.setBaudRate(9600);
        ...
        // 创建ModbusMaster工厂实例
        return new ModbusFactory().createRtuMaster((SerialPortWrapper) serialParameters);
    }
